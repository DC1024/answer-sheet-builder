...
| 🎯 **Corner registration marks** | **Square** or **triangle** marks with adjustable size (2–10mm). Every face that **has content** automatically gets four corner marks; blank faces get none. Forced to render in print for scanner alignment. |
...
| 🪪 **Student info bar** | Add/remove handwritten fields (class / name / exam number); the **exam-number grid** uses bracket squares with **digits printed inside** (up to 20 digits, auto-scaled beyond 12) and arranges the fields to its right to save space. |
