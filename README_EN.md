...
| ✎ **Nested blank items** | Supports nested items like `11(1)`, `11(2)①/②`, with **layer-aware auto-numbering** (sub `(1)(2)` → sub-sub `①②` → deeper `a) b)`); **items keep filling the same line until it is full**, then wrap. Each blank's **length** and **row spacing** are adjustable. |
