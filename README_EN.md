...
| 🔤 **Bubble or handwritten** | Choice questions support **bracketed `[A]` bubbling** or **handwritten lines**; any option count is allowed (beyond 26, labels continue `AA`, `AB`, …). The column count is derived from the **measured width of a real question at render time**, so **every question fits entirely on one line**; a question that doesn't fit moves to the next row — nothing ever overflows. **Row spacing (mm)** is configurable and is **identical from the title to the first row and between rows**. |
