...
| 🎯 **四角定位点** | 支持**方块 / 三角**两种样式，边长可调（2–10mm）。每一面只要**有题目内容**就自动补四角定位点，空白面不加；打印时强制输出底色，供阅卷机定位。 |
...
| 🪪 **考生信息栏** | 可增删「班级 / 姓名 / 考号」等手写栏；**考号填涂区**为中括号方块、**框内直接显示数字**（位数上限 20，超 12 位整表等比缩小），启用后手写栏自动排布在其右侧，节省空间。 |
